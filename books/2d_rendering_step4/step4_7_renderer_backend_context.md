...
| ヘッダ名 | 外部公開API | 役割 |
| ------- | --------- | ---- |
| renderer_backend_context/context.h     | renderer_backend_initialize                 | レンダラーバックエンドのメモリを確保し、初期化を行う                   |
|                                        | renderer_backend_destroy                    | レンダラーバックエンドの終了処理を行う                               |
| renderer_backend_context/shader.h      | renderer_backend_shader_create              | シェーダーハンドル構造体インスタンスのメモリを確保し0で初期化する        |
...
| renderer_backend_context/context_vao.h | renderer_backend_vertex_array_create        | VAO内部状態管理構造体インスタンスのメモリを確保する                    |
...
| renderer_backend_context/context_vbo.h | renderer_backend_vertex_buffer_create       | VBO内部状態管理構造体インスタンスのメモリを確保する                    |
