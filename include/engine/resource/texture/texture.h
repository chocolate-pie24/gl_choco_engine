 ...
 *
 * @note 以下のテクスチャ名称は特殊(ビルトインテクスチャ)で、filepath_, extension_はNULLでok
 * - test_texture_red
 * - test_texture_green
 * - test_texture_blue
 ...
 *
 * @param[in,out] texture_ ロード対象テクスチャ名称を格納するテクスチャ構造体インスタンスへのポインタ
 * @param[in] filepath_ ファイルパス
 * @param[in] extension_ ファイル拡張子
 *
