// TODO: 点群に対するAABBのように毎フレーム変化するgeometryの場合、name, idは同一でvertex count, verticesのみを更新するupdate APIが必要, これをやるためにはvertex bufferの再配置APIが必要(FreeList実装後)
#ifndef GLCE_ENGINE_SYSTEMS_RENDERER_RESOURCE_PIPELINES_LINE_MESH_GEOMETRY_H
#define GLCE_ENGINE_SYSTEMS_RENDERER_RESOURCE_PIPELINES_LINE_MESH_GEOMETRY_H
