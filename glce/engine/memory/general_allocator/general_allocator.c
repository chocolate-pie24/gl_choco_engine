...
static const char* rslt_to_str(general_allocator_result_t rslt_);
static general_allocator_result_t result_convert_free_list_allocator(free_list_allocator_result_t rslt_);
...
general_allocator_result_t general_allocator_create(void) {
    ...
    ERROR_MESSAGE("general_allocator_create(%s) - Undefined build config.", rslt_to_str(ret));
    goto cleanup;
#endif
    if(NULL == s_general_allocator.memory_pool) {
        ret = GENERAL_ALLOCATOR_NO_MEMORY;
        ERROR_MESSAGE("general_allocator_create(%s) - Failed to allocate memory for memory pool.", rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(FREE_LIST_ALLOCATOR_SUCCESS != ret_free_list_allocator) {
        ...
        ERROR_MESSAGE("general_allocator_create(%s) - free_list_allocator_initialize failed.", rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(!general_allocator_is_valid()) {
        ...
        ERROR_MESSAGE("general_allocator_create(%s) - Postcondition validation failed for 's_general_allocator'.", rslt_to_str(ret));
        goto cleanup;
    }
#endif
...
void general_allocator_destroy(void) {
...
#if defined(DEBUG_BUILD) || defined(TEST_BUILD)
    if(!general_allocator_is_valid()) {
        ERROR_MESSAGE("general_allocator_destroy(%s) - Precondition validation failed for 's_general_allocator'.", rslt_to_str(GENERAL_ALLOCATOR_DATA_CORRUPTED));
        return;
    }
#endif
    ...
    if(FREE_LIST_ALLOCATOR_SUCCESS != ret_free_list_allocator) {
        ERROR_MESSAGE("general_allocator_destroy(%s) - free_list_allocator_deinitialize failed.", rslt_to_str(GENERAL_ALLOCATOR_DATA_CORRUPTED));
        return;
    }
#if defined(GLCE_BUILD_MEMORY_POLICY_DESKTOP)
    free(s_general_allocator.memory_pool);
...
general_allocator_result_t general_allocator_allocate(size_t allocation_size_, general_allocator_memory_tag_t memory_tag_, void** out_ptr_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(out_ptr_, ret, GENERAL_ALLOCATOR_INVALID_ARGUMENT, rslt_to_str(GENERAL_ALLOCATOR_INVALID_ARGUMENT), "general_allocator_allocate", "out_ptr_")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(*out_ptr_, ret, GENERAL_ALLOCATOR_BAD_OPERATION, rslt_to_str(GENERAL_ALLOCATOR_BAD_OPERATION), "general_allocator_allocate", "*out_ptr_")
    if(0 == allocation_size_) {
        ret = GENERAL_ALLOCATOR_INVALID_ARGUMENT;
        ERROR_MESSAGE("general_allocator_allocate(%s) - Provided allocation_size_ is not valid.", rslt_to_str(ret));
        goto cleanup;
    }
    if(!memory_tag_is_valid(memory_tag_)) {
        ret = GENERAL_ALLOCATOR_INVALID_ARGUMENT;
        ERROR_MESSAGE("general_allocator_allocate(%s) - Provided memory_tag_ is not valid.", rslt_to_str(ret));
        goto cleanup;
    }
#if defined(DEBUG_BUILD) || defined(TEST_BUILD)
    if(!general_allocator_is_valid()) {
        ret = GENERAL_ALLOCATOR_DATA_CORRUPTED;
        ERROR_MESSAGE("general_allocator_allocate(%s) - Precondition validation failed for 's_general_allocator'.", rslt_to_str(ret));
        goto cleanup;
    }
#endif
    ...
    if(FREE_LIST_ALLOCATOR_SUCCESS != ret_free_list_allocator) {
        ret = result_convert_free_list_allocator(ret_free_list_allocator);
        ERROR_MESSAGE("general_allocator_allocate(%s) - free_list_allocator_allocate failed.", rslt_to_str(ret));
        goto cleanup;
    }
...
void general_allocator_free(void** ptr_, general_allocator_memory_tag_t memory_tag_) {
    ...
    if(NULL == ptr_ || NULL == *ptr_) {
        ERROR_MESSAGE("general_allocator_free(%s) - Provided ptr_ is not valid.", rslt_to_str(GENERAL_ALLOCATOR_INVALID_ARGUMENT));
        return;
    }
    if(!memory_tag_is_valid(memory_tag_)) {
        ERROR_MESSAGE("general_allocator_free(%s) - Provided memory_tag_ is not valid.", rslt_to_str(GENERAL_ALLOCATOR_INVALID_ARGUMENT));
        return;
    }
#if defined(DEBUG_BUILD) || defined(TEST_BUILD)
    if(!general_allocator_is_valid()) {
        ERROR_MESSAGE("general_allocator_free(%s) - Precondition validation failed for 's_general_allocator'.", rslt_to_str(GENERAL_ALLOCATOR_DATA_CORRUPTED));
        return;
    }
#endif
    ...
    if(FREE_LIST_ALLOCATOR_SUCCESS != ret_free_list_allocator) {
        ERROR_MESSAGE("general_allocator_free(%s) - general allocator is corrupted.", rslt_to_str(GENERAL_ALLOCATOR_DATA_CORRUPTED));
        return;
    }
