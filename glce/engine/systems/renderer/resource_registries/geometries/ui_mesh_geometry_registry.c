...
resource_registry_result_t ui_mesh_geometry_registry_initialize(size_t max_geometry_count_, linear_alloc_t* allocator_, ui_mesh_geometry_registry_t** out_registry_) {
    ...
    if((SIZE_MAX / max_geometry_count_) < sizeof(ui_mesh_geometry_t*)) {
        ret = RESOURCE_REGISTRY_OVERFLOW;
        ERROR_MESSAGE("ui_mesh_geometry_registry_initialize(%s) - Allocation size overflow while calculating geometry pointer array size. target=geometries, elem_type=ui_mesh_geometry_t*, elem_count=%zu, elem_size=%zu, size_max=%zu", resource_registry_rslt_to_str(ret), max_geometry_count_, sizeof(ui_mesh_geometry_t*), SIZE_MAX);
        goto cleanup;
    }
    ret_linear_alloc = linear_allocator_allocate(allocator_, sizeof(ui_mesh_geometry_t*) * max_geometry_count_, alignof(ui_mesh_geometry_t*), (void**)&tmp_geometry_array);
    if(LINEAR_ALLOC_SUCCESS != ret_linear_alloc) {
        ret = resource_registry_rslt_convert_linear_alloc(ret_linear_alloc);
        ERROR_MESSAGE("ui_mesh_geometry_registry_initialize(%s) - Failed to allocate geometry pointer array. target=geometries, elem_type=ui_mesh_geometry_t*, elem_count=%zu, elem_size=%zu, bytes=%zu, align=%zu", resource_registry_rslt_to_str(ret), max_geometry_count_, sizeof(ui_mesh_geometry_t*), sizeof(ui_mesh_geometry_t*) * max_geometry_count_, alignof(ui_mesh_geometry_t*));
        goto cleanup;
    }

    if((SIZE_MAX / max_geometry_count_) < sizeof(vbo_range_t)) {
        ret = RESOURCE_REGISTRY_OVERFLOW;
        ERROR_MESSAGE("ui_mesh_geometry_registry_initialize(%s) - Allocation size overflow while calculating vertex offset array size. target=vertex_offsets, elem_type=size_t, elem_count=%zu, elem_size=%zu, size_max=%zu", resource_registry_rslt_to_str(ret), max_geometry_count_, sizeof(vbo_range_t), SIZE_MAX);
        goto cleanup;
    }
...
bool ui_mesh_geometry_registry_find(const ui_mesh_geometry_registry_t* registry_, const char* name_) {
    ...
    if(!internal_state_is_valid(registry_)) {
        ERROR_MESSAGE("ui_mesh_geometry_registry_find(%s) - Registry internal state check failed. operation=find, target=ui_mesh_geometry_registry_t, query_name='%s'", resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), name_);
        return false;
    }
...
resource_registry_result_t ui_mesh_geometry_registry_id_get(const ui_mesh_geometry_registry_t* registry_, const char* name_, int16_t* out_geometry_id_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(internal_state_is_valid(registry_), ret, RESOURCE_REGISTRY_DATA_CORRUPTED, resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), "ui_mesh_geometry_registry_id_get", "registry_")

    if(!find_by_name(registry_, name_, &tmp_id)) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_id_get(%s) - Failed to get ui mesh geometry id. reason=not_registered, query_name='%s'", resource_registry_rslt_to_str(ret), name_);
        goto cleanup;
    }
...
resource_registry_result_t ui_mesh_geometry_registry_vbo_range_get(const ui_mesh_geometry_registry_t* registry_, int16_t geometry_id_, vbo_range_t* out_vbo_range_) {
    resource_registry_result_t ret = RESOURCE_REGISTRY_INVALID_ARGUMENT;
    ...
    if(NULL == registry_->geometries[geometry_id_]) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_vbo_range_get(%s) - Failed to get ui mesh geometry draw range. reason=not_registered, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }
...
resource_registry_result_t ui_mesh_geometry_registry_register(ui_mesh_geometry_registry_t* registry_, const ui_mesh_geometry_t* geometry_, const vbo_range_t* vbo_range_, int16_t* out_geometry_id_) {
    resource_registry_result_t ret = RESOURCE_REGISTRY_INVALID_ARGUMENT;
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(0 != vbo_range_->allocation_info.range_allocation.allocated_size, ret, RESOURCE_REGISTRY_BAD_OPERATION, resource_registry_rslt_to_str(RESOURCE_REGISTRY_BAD_OPERATION), "ui_mesh_geometry_registry_register", "vbo_range_->allocation_info.range_allocation.allocated_size")
    IF_ARG_FALSE_GOTO_CLEANUP(0 != vbo_range_->draw_range.vertex_count, ret, RESOURCE_REGISTRY_BAD_OPERATION, resource_registry_rslt_to_str(RESOURCE_REGISTRY_BAD_OPERATION), "ui_mesh_geometry_registry_register", "vbo_range_->draw_range.vertex_count")
    ...
    if(NULL == name) {
        ...
        ERROR_MESSAGE("ui_mesh_geometry_registry_register(%s) - Failed to register ui mesh geometry. reason=name_get_failed", resource_registry_rslt_to_str(ret));
        goto cleanup;
    }
    if(find_by_name(registry_, name, &unused_index)) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_register(%s) - Failed to register ui mesh geometry. reason=already_registered, geometry_name='%s'", resource_registry_rslt_to_str(ret), name);
        goto cleanup;
    }
    ...
    for(size_t i = 0; i != registry_->max_geometry_count; ++i) {
        if(NULL == registry_->geometries[i]) {
            ...
            if(RESOURCE_SUCCESS != ret_resource) {
                ret = resource_registry_rslt_convert_resource(ret_resource);
                ERROR_MESSAGE("ui_mesh_geometry_registry_register(%s) - Failed to register ui mesh geometry. reason=clone_failed, geometry_name='%s'", resource_registry_rslt_to_str(ret), name);
                goto cleanup;
            }
    ...
    if(!found_free_slot) {
        ...
        ERROR_MESSAGE("ui_mesh_geometry_registry_register(%s) - Failed to register ui mesh geometry. reason=registry_full, geometry_name='%s', max_geometry_count=%zu", resource_registry_rslt_to_str(ret), name, registry_->max_geometry_count);
        goto cleanup;
    }
...
resource_registry_result_t ui_mesh_geometry_registry_unregister(ui_mesh_geometry_registry_t* registry_, int16_t geometry_id_) {
    resource_registry_result_t ret = RESOURCE_REGISTRY_INVALID_ARGUMENT;
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(internal_state_is_valid(registry_), ret, RESOURCE_REGISTRY_DATA_CORRUPTED, resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), "ui_mesh_geometry_registry_unregister", "registry_")
    ...
    if(NULL == registry_->geometries[geometry_id_]) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_unregister(%s) - Failed to unregister ui mesh geometry. reason=not_registered, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }
...
static bool find_by_name(const ui_mesh_geometry_registry_t* registry_, const char* name_, size_t* out_index_) {
    ...
    for(size_t i = 0; i != registry_->max_geometry_count; ++i) {
        if(NULL != registry_->geometries[i]) {
            tmp_name = ui_mesh_geometry_name_get(registry_->geometries[i]);
