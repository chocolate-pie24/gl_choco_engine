...
bool point_mesh_geometry_registry_find(const point_mesh_geometry_registry_t* registry_, const char* name_) {
    ...
    if(!point_mesh_geometry_registry_is_valid(registry_)) {
        ERROR_MESSAGE("point_mesh_geometry_registry_find(%s) - point_mesh_geometry_registry_t internal state is corrupted.", resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED));
        return false;
    }
    if('\0' == name_[0]) {
        ERROR_MESSAGE("point_mesh_geometry_registry_find(%s) - provided resource name is not valid.", resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT));
        return false;
    }
