...
typedef struct registry_entry {
    ...
    line_mesh_geometry_t* cpu_resource;
...
resource_registry_result_t line_mesh_geometry_registry_initialize(size_t max_geometry_count_, linear_alloc_t* allocator_, line_mesh_geometry_registry_t** out_registry_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(UINT16_MAX >= max_geometry_count_, ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "line_mesh_geometry_registry_initialize", "max_geometry_count_")
    ...
    if((SIZE_MAX / max_geometry_count_) < sizeof(registry_entry_t)) {
        ...
        ERROR_MESSAGE("line_mesh_geometry_registry_initialize(%s) - overflow.", resource_registry_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(LINEAR_ALLOC_SUCCESS != ret_linear_alloc) {
        ret = resource_registry_rslt_convert_linear_alloc(ret_linear_alloc);
        ERROR_MESSAGE("line_mesh_geometry_registry_initialize(%s) - allocation failed.", resource_registry_rslt_to_str(ret));
        goto cleanup;
    }
...
bool line_mesh_geometry_registry_find(const line_mesh_geometry_registry_t* registry_, const char* name_) {
    ...
    if(!line_mesh_geometry_registry_is_valid(registry_)) {
        ERROR_MESSAGE("line_mesh_geometry_registry_find(%s) - line_mesh_geometry_registry_t internal state is corrupted.", resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED));
        return false;
    }
    if('\0' == name_[0]) {
        ERROR_MESSAGE("line_mesh_geometry_registry_find(%s) - provided resource name is not valid.", resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT));
        return false;
    }
...
// TODO: cpu_resource, allocation descriptorのvalidation
