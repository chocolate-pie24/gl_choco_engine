...
renderer_backend_result_t renderer_backend_create(linear_alloc_t* allocator_, renderer_backend_context_t** out_renderer_backend_context_) {
    ...
    if(LINEAR_ALLOC_SUCCESS != ret_linear_alloc) {
        ...
        ERROR_MESSAGE("renderer_backend_create(%s) - Failed to allocate memory for renderer backend context.", renderer_backend_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(NULL == tmp_context->shader_vtable) {
        ...
        ERROR_MESSAGE("renderer_backend_create(%s) - Failed to get shader vtable.", renderer_backend_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(NULL == tmp_context->vao_vtable) {
        // ここは引数チェックが事前にされているので通らないため、カバレッジは100にならないが、許容
        ret = RENDERER_BACKEND_RUNTIME_ERROR;
        ERROR_MESSAGE("renderer_backend_create(%s) - Failed to get vao vtable.", renderer_backend_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(NULL == tmp_context->vbo_vtable) {
        // ここは引数チェックが事前にされているので通らないため、カバレッジは100にならないが、許容
        ret = RENDERER_BACKEND_RUNTIME_ERROR;
        ERROR_MESSAGE("renderer_backend_create(%s) - Failed to get vbo vtable.", renderer_backend_rslt_to_str(ret));
        goto cleanup;
    }
