...
static event_system_result_t event_storage_initialize(const event_system_config_t* config_, linear_allocator_t* linear_allocator_, window_event_storage_t* window_event_storage_, keyboard_event_storage_t* keyboard_event_storage_, mouse_event_storage_t* mouse_event_storage_);
...
event_system_result_t event_system_create(const event_system_config_t* config_, linear_allocator_t* linear_allocator_, platform_system_t* platform_system_, event_system_t** out_event_system_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(config_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_system_create", "config_")
    IF_ARG_NULL_GOTO_CLEANUP(linear_allocator_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_system_create", "linear_allocator_")
    IF_ARG_NULL_GOTO_CLEANUP(platform_system_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_system_create", "platform_system_")
    IF_ARG_NULL_GOTO_CLEANUP(out_event_system_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_system_create", "out_event_system_")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(*out_event_system_, ret, EVENT_SYSTEM_BAD_OPERATION, event_system_result_to_str(EVENT_SYSTEM_BAD_OPERATION), "event_system_create", "*out_event_system_")
    if(!event_system_config_is_valid(config_)) {
        ret = EVENT_SYSTEM_INVALID_ARGUMENT;
        ERROR_MESSAGE("event_system_create(%s) - Provided config_ is not valid.", event_system_result_to_str(ret));
        goto cleanup;
    }

    // event system
    ret_linear_allocator = linear_allocator_allocate(linear_allocator_, sizeof(event_system_t), alignof(event_system_t), (void**)&tmp_event_system);
    ...
    ret = event_storage_initialize(config_, linear_allocator_, &tmp_event_system->window_event_storage, &tmp_event_system->keyboard_event_storage, &tmp_event_system->mouse_event_storage);
    if(EVENT_SYSTEM_SUCCESS != ret) {
        ERROR_MESSAGE("event_system_create(%s) - event_storage_initialize failed.", event_system_result_to_str(ret));
        goto cleanup;
    }
    ...
    if(EVENT_SYSTEM_SUCCESS != ret) {
        ERROR_MESSAGE("event_system_create(%s) - event_view_refresh failed.", event_system_result_to_str(ret));
        goto cleanup;
    }
    ...
    if(!event_system_is_valid(tmp_event_system)) {
        ...
        ERROR_MESSAGE("event_system_create(%s) - Postcondition validation failed for 'tmp_event_system'.", event_system_result_to_str(ret));
        goto cleanup;
    }
#endif
...
void event_system_deinitialize(event_system_t* event_system_) {
    ...
    if(!event_system_is_valid(event_system_)) {
        ERROR_MESSAGE("event_system_deinitialize(%s) - Precondition validation failed for 'event_system_'.", event_system_result_to_str(EVENT_SYSTEM_DATA_CORRUPTED));
        return;
    }
#endif
...
event_system_result_t event_system_update(event_system_t* event_system_, const engine_event_view_t** out_event_view_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(event_system_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_system_update", "event_system_")
    IF_ARG_NULL_GOTO_CLEANUP(out_event_view_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_system_update", "out_event_view_")
#if defined(DEBUG_BUILD) || defined(TEST_BUILD)
    if(!is_valid_shallow(event_system_)) {
        ret = EVENT_SYSTEM_DATA_CORRUPTED;
        ERROR_MESSAGE("event_system_update(%s) - Precondition validation failed for 'event_system_'.", event_system_result_to_str(ret));
        goto cleanup;
    }
#endif
    ...
    if(PLATFORM_SYSTEM_SUCCESS != ret_platform_system) {
        ret = event_system_result_convert_platform_system(ret_platform_system);
        ERROR_MESSAGE("event_system_update(%s) - platform_system_update failed.", event_system_result_to_str(ret));
        goto cleanup;
    }
    ...
    if(platform_event_view->keyboard_event_count > event_system_->keyboard_event_storage.max_event_count) {
        ...
        ERROR_MESSAGE("event_system_update(%s) - keyboard event count limit exceeded.", event_system_result_to_str(ret));
        goto cleanup;
    }

    if(platform_event_view->mouse_event_count > event_system_->mouse_event_storage.max_event_count) {
        ret = EVENT_SYSTEM_LIMIT_EXCEEDED;
        ERROR_MESSAGE("event_system_update(%s) - mouse event count limit exceeded.", event_system_result_to_str(ret));
        goto cleanup;
    }

    if(platform_event_view->window_event_count > event_system_->window_event_storage.max_event_count) {
        ret = EVENT_SYSTEM_LIMIT_EXCEEDED;
        ERROR_MESSAGE("event_system_update(%s) - window event count limit exceeded.", event_system_result_to_str(ret));
        goto cleanup;
    }
    ...
    for(size_t i = 0; i != platform_event_view->keyboard_event_count; ++i) {
        ...
        if(EVENT_SYSTEM_SUCCESS != ret) {
            ERROR_MESSAGE("event_system_update(%s) - keyboard_event_storage_push failed.", event_system_result_to_str(ret));
            goto cleanup;
        }
    }
    ...
    for(size_t i = 0; i != platform_event_view->mouse_event_count; ++i) {
        ...
        if(EVENT_SYSTEM_SUCCESS != ret) {
            ERROR_MESSAGE("event_system_update(%s) - mouse_event_storage_push failed.", event_system_result_to_str(ret));
            goto cleanup;
        }
    }
    ...
    for(size_t i = 0; i != platform_event_view->window_event_count; ++i) {
        ...
        if(EVENT_SYSTEM_SUCCESS != ret) {
            ERROR_MESSAGE("event_system_update(%s) - window_event_storage_push failed.", event_system_result_to_str(ret));
            goto cleanup;
        }
    }
    ...
    if(EVENT_SYSTEM_SUCCESS != ret) {
        ERROR_MESSAGE("event_system_update(%s) - event_view_refresh failed.", event_system_result_to_str(ret));
        goto cleanup;
    }
...
static event_system_result_t event_storage_initialize(const event_system_config_t* config_, linear_allocator_t* linear_allocator_, window_event_storage_t* window_event_storage_, keyboard_event_storage_t* keyboard_event_storage_, mouse_event_storage_t* mouse_event_storage_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(config_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_storage_initialize", "config_")
    IF_ARG_NULL_GOTO_CLEANUP(linear_allocator_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_storage_initialize", "linear_allocator_")
    IF_ARG_NULL_GOTO_CLEANUP(window_event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_storage_initialize", "window_event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(keyboard_event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_storage_initialize", "keyboard_event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(mouse_event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_storage_initialize", "mouse_event_storage_")

    // window event
    if((SIZE_MAX / config_->max_window_event_count) < sizeof(window_event_t)) {
        ret = EVENT_SYSTEM_OVERFLOW;
        ERROR_MESSAGE("event_storage_initialize(%s) - window event array size overflow.", event_system_result_to_str(ret));
        goto cleanup;
    }
    ...
    if((SIZE_MAX / config_->max_keyboard_event_count) < sizeof(keyboard_event_t)) {
        ret = EVENT_SYSTEM_OVERFLOW;
        ERROR_MESSAGE("event_storage_initialize(%s) - keyboard event array size overflow.", event_system_result_to_str(ret));
        goto cleanup;
    }
    ...
    if((SIZE_MAX / config_->max_mouse_event_count) < sizeof(mouse_event_t)) {
        ret = EVENT_SYSTEM_OVERFLOW;
        ERROR_MESSAGE("event_storage_initialize(%s) - mouse event array size overflow.", event_system_result_to_str(ret));
        goto cleanup;
    }
...
static event_system_result_t event_view_refresh(bool window_close_requested_, const window_event_storage_t* window_event_storage_, const keyboard_event_storage_t* keyboard_event_storage_, const mouse_event_storage_t* mouse_event_storage_, engine_event_view_t* event_view_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(window_event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_view_refresh", "window_event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(keyboard_event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_view_refresh", "keyboard_event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(mouse_event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_view_refresh", "mouse_event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(event_view_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "event_view_refresh", "event_view_")
...
static event_system_result_t window_event_storage_push(window_event_storage_t* event_storage_, const window_event_t* event_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "window_event_storage_push", "event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(event_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "window_event_storage_push", "event_")
    if(event_storage_->current_event_count >= event_storage_->max_event_count) {
        ret = EVENT_SYSTEM_LIMIT_EXCEEDED;
        ERROR_MESSAGE("window_event_storage_push(%s) - window event storage limit exceeded.", event_system_result_to_str(ret));
        goto cleanup;
    }
...
static event_system_result_t keyboard_event_storage_push(keyboard_event_storage_t* event_storage_, const keyboard_event_t* event_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "keyboard_event_storage_push", "event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(event_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "keyboard_event_storage_push", "event_")
    if(event_storage_->current_event_count >= event_storage_->max_event_count) {
        ret = EVENT_SYSTEM_LIMIT_EXCEEDED;
        ERROR_MESSAGE("keyboard_event_storage_push(%s) - keyboard event storage limit exceeded.", event_system_result_to_str(ret));
        goto cleanup;
    }
...
static event_system_result_t mouse_event_storage_push(mouse_event_storage_t* event_storage_, const mouse_event_t* event_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(event_storage_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "mouse_event_storage_push", "event_storage_")
    IF_ARG_NULL_GOTO_CLEANUP(event_, ret, EVENT_SYSTEM_INVALID_ARGUMENT, event_system_result_to_str(EVENT_SYSTEM_INVALID_ARGUMENT), "mouse_event_storage_push", "event_")
    if(event_storage_->current_event_count >= event_storage_->max_event_count) {
        ret = EVENT_SYSTEM_LIMIT_EXCEEDED;
        ERROR_MESSAGE("mouse_event_storage_push(%s) - mouse event storage limit exceeded.", event_system_result_to_str(ret));
        goto cleanup;
    }
