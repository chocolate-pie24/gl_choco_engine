...
resource_result_t ui_mesh_geometry_create_from_vertices(size_t vertex_count_, const ui_vertex_t* vertices_, ui_mesh_geometry_t** geometry_) {
    ...
    ret = ui_mesh_geometry_initialize_from_vertices(vertex_count_, vertices_, tmp_geometry);
    if(RESOURCE_SUCCESS != ret) {
...
resource_result_t ui_mesh_geometry_initialize_from_vertices(size_t vertex_count_, const ui_vertex_t* vertices_, ui_mesh_geometry_t* geometry_) {
    ...
    if(6 != vertex_count_) {
        ret = RESOURCE_INVALID_ARGUMENT;
        ERROR_MESSAGE("ui_mesh_geometry_initialize_from_vertices(%s) - Provided vertex_count_ is not valid.", resource_rslt_to_str(ret));
        goto cleanup;
    }
    if(0 != geometry_->vertex_count) {
        ret = RESOURCE_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_initialize_from_vertices(%s) - Provided geometry_ is already initialized.", resource_rslt_to_str(ret));
        goto cleanup;
    }
...
resource_result_t ui_mesh_geometry_clone(const ui_mesh_geometry_t* src_, ui_mesh_geometry_t** out_geometry_) {
    ...
    if(0 != src_->vertex_count) {
        ret = ui_mesh_geometry_initialize_from_vertices(src_->vertex_count, src_->vertices, tmp_geometry);
        if(RESOURCE_OVERFLOW == ret) {
            ret = RESOURCE_DATA_CORRUPTED;
            ERROR_MESSAGE("ui_mesh_geometry_clone(%s) - src_ internal state is corrupted: overflow occurred while deep-copying vertices.", resource_rslt_to_str(ret));
            goto cleanup;
