...
resource_result_t lit_mesh_geometry_create_from_vertices(size_t vertex_count_, const point_normal_vertex_t* vertices_, lit_mesh_geometry_t** out_geometry_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(out_geometry_, ret, RESOURCE_INVALID_ARGUMENT, resource_rslt_to_str(RESOURCE_INVALID_ARGUMENT), "lit_mesh_geometry_create_from_vertices", "out_geometry_")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(*out_geometry_, ret, RESOURCE_BAD_OPERATION, resource_rslt_to_str(RESOURCE_BAD_OPERATION), "lit_mesh_geometry_create_from_vertices", "*out_geometry_")
    if(0 == vertex_count_ || 0 != (vertex_count_ % 3)) {
        ret = RESOURCE_INVALID_ARGUMENT;
        ERROR_MESSAGE("lit_mesh_geometry_create_from_vertices(%s) - Provided vertex_count_ is not valid.", resource_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(MEMORY_SYSTEM_SUCCESS != ret_memory_system) {
        ...
        ERROR_MESSAGE("lit_mesh_geometry_create_from_vertices(%s) - Failed to allocate lit_mesh_geometry_t instance.", resource_rslt_to_str(ret));
        goto cleanup;
    }
...
static resource_result_t initialize_from_vertices(lit_mesh_geometry_t* geometry_, size_t vertex_count_, const point_normal_vertex_t* vertices_) {
    ...
    if((SIZE_MAX / vertex_count_) < sizeof(point_normal_vertex_t)) {
        ...
        ERROR_MESSAGE("initialize_from_vertices(%s) - CPU-side vertex array size overflow. vertex_count = %zu, vertex_size = %zu.", resource_rslt_to_str(ret), vertex_count_, sizeof(point_normal_vertex_t));
        goto cleanup;
    }
    ret_memory_system = choco_memory_allocate(sizeof(point_normal_vertex_t) * vertex_count_, MEMORY_TAG_GEOMETRY, (void**)&tmp_vertices);
    if(MEMORY_SYSTEM_SUCCESS != ret_memory_system) {
        ret = resource_rslt_convert_choco_memory(ret_memory_system);
        ERROR_MESSAGE("initialize_from_vertices(%s) - Failed to allocate CPU-side vertex array. vertex_count = %zu, vertex_size = %zu.", resource_rslt_to_str(ret), vertex_count_, sizeof(point_normal_vertex_t));
        goto cleanup;
    }
