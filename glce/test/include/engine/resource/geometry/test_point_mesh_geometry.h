 ...
 *
 */
#ifndef GLCE_TEST_ENGINE_RESOURCE_GEOMETRY_TEST_POINT_MESH_GEOMETRY_H
#define GLCE_TEST_ENGINE_RESOURCE_GEOMETRY_TEST_POINT_MESH_GEOMETRY_H
 ...
 * @param[in] config_ テスト設定値構造体インスタンスへのポインタ
 */
void test_point_mesh_geometry_vertices_get_config_set(const test_call_control_t* config_);

/**
 * @brief point_mesh_geometry_vertex_count_get()APIに対して失敗注入設定を行う
 *
 * @note API呼び出し回数についてはコピーされず、point_mesh_geometry内で管理している値が保持される
 *
 * @param[in] config_ テスト設定値構造体インスタンスへのポインタ
 */
void test_point_mesh_geometry_vertex_count_get_config_set(const test_call_control_t* config_);

/**
 * @brief point_mesh_geometryが内部で管理するテスト設定値を全て初期化し、テスト用の出力強制制御をなくす
 *
 */
void test_point_mesh_geometry_config_reset(void);

/**
 * @brief point_mesh_geometry保有APIのテストを行う
 *
 */
void test_point_mesh_geometry(void);
