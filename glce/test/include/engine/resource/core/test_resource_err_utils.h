 ...
 *
 */
#ifndef GLCE_TEST_ENGINE_RESOURCE_CORE_TEST_RESOURCE_ERR_UTILS_H
#define GLCE_TEST_ENGINE_RESOURCE_CORE_TEST_RESOURCE_ERR_UTILS_H
...
/**
 * @brief resource_rslt_convert_geometry_primitive()APIに対して失敗注入設定を行う
 *
 * @note API呼び出し回数についてはコピーされず、resource_err_utils内で管理している値が保持される
 *
 * @param[in] config_ テスト設定値構造体インスタンスへのポインタ
 */
void test_resource_rslt_convert_geometry_primitive_config_set(const test_call_control_t* config_);
