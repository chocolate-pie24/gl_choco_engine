...
application_result_t application_create(void) {
    ...
    // begin Simulation
    // Application State
    ret_memory_system = choco_memory_allocate(sizeof(*tmp_app_state), MEMORY_TAG_SYSTEM, (void**)&tmp_app_state);
    if(MEMORY_SYSTEM_SUCCESS != ret_memory_system) {
        ret = application_result_convert_memory_system(ret_memory_system);
    ...
    tmp_app_state->linear_allocator = NULL;
    linear_allocator_preinit(&tmp_app_state->linear_alloc_mem_req, &tmp_app_state->linear_alloc_align_req);
    ret_memory_system = choco_memory_allocate(tmp_app_state->linear_alloc_mem_req, MEMORY_TAG_SYSTEM, (void**)&tmp_app_state->linear_allocator);
    if(MEMORY_SYSTEM_SUCCESS != ret_memory_system) {
        ret = application_result_convert_memory_system(ret_memory_system);
    ...
    ret_memory_system = choco_memory_allocate(tmp_app_state->linear_alloc_pool_size, MEMORY_TAG_SYSTEM, &tmp_app_state->linear_alloc_pool);
    if(MEMORY_SYSTEM_SUCCESS != ret_memory_system) {
        ret = application_result_convert_memory_system(ret_memory_system);
    ...
    ret_linear_allocator = linear_allocator_initialize(tmp_app_state->linear_allocator, tmp_app_state->linear_alloc_pool_size, tmp_app_state->linear_alloc_pool);
    ...
    ret_platform_system = platform_system_create(&tmp_app_state->platform_system_config, tmp_app_state->linear_allocator, &tmp_app_state->frame_state.framebuffer_width, &tmp_app_state->frame_state.framebuffer_height, &tmp_app_state->platform_system);
    ...
    ret_event_system = event_system_create(&tmp_app_state->event_system_config, tmp_app_state->linear_allocator, tmp_app_state->platform_system, &tmp_app_state->event_system);
    ...
    ret = application_flight_camera_create(8, tmp_app_state->linear_allocator, tmp_app_state->frame_state.framebuffer_width, tmp_app_state->frame_state.framebuffer_height, &tmp_app_state->flight_camera);
    if(APPLICATION_SUCCESS != ret) {
    ...
    ret = application_renderer_create(&tmp_app_state->renderer_config, tmp_app_state->linear_allocator, fs_path_fullpath_get(tmp_app_state->executable_directory), "../../assets/shaders/test_shader/", &tmp_app_state->renderer);
    if(APPLICATION_SUCCESS != ret) {
...
cleanup:
    if(APPLICATION_SUCCESS != ret) {
        if(NULL != tmp_app_state) {
            ...
            if(NULL != tmp_app_state->linear_allocator) {
                choco_memory_free(tmp_app_state->linear_allocator, tmp_app_state->linear_alloc_mem_req, MEMORY_TAG_SYSTEM);
            }
...
void application_destroy(void) {
    ...
    if(NULL != s_app_state->linear_allocator) {
        choco_memory_free(s_app_state->linear_allocator, s_app_state->linear_alloc_mem_req, MEMORY_TAG_SYSTEM);
        s_app_state->linear_allocator = NULL;
    }
...
static application_result_t app_state_update(void) {
    ...
    if(!application_frame_state_is_valid(&s_app_state->frame_state)) {
        ...
        ERROR_MESSAGE("app_state_update(%s) - Postcondition validation failed for 's_app_state->frame_state'.", application_result_to_str(ret));
        goto cleanup;
    }
#endif
...
static application_result_t point_mesh_geometry_import(app_state_t* app_state_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(app_state_, ret, APPLICATION_INVALID_ARGUMENT, application_result_to_str(APPLICATION_INVALID_ARGUMENT), "point_mesh_geometry_import", "app_state_")
...
static application_result_t ui_mesh_geometry_import(app_state_t* app_state_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(app_state_, ret, APPLICATION_INVALID_ARGUMENT, application_result_to_str(APPLICATION_INVALID_ARGUMENT), "ui_mesh_geometry_import", "app_state_")
...
static application_result_t lit_mesh_geometry_import(app_state_t* app_state_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(app_state_, ret, APPLICATION_INVALID_ARGUMENT, application_result_to_str(APPLICATION_INVALID_ARGUMENT), "lit_mesh_geometry_import", "app_state_")
...
static application_result_t ui_mesh_textures_import(app_state_t* app_state_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(app_state_, ret, APPLICATION_INVALID_ARGUMENT, application_result_to_str(APPLICATION_INVALID_ARGUMENT), "ui_mesh_textures_import", "app_state_")
...
static application_result_t executable_directory_get(app_state_t* app_state_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(app_state_, ret, APPLICATION_INVALID_ARGUMENT, application_result_to_str(APPLICATION_INVALID_ARGUMENT), "executable_directory_get", "app_state_")
