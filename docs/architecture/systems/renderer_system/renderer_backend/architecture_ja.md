...
`Renderer Backend`が提供する機能を大別してVAO, VBO, Shader, Textureの4つに分類し、それぞれグラフィックスAPIの差異を吸収するためのInterfaceを設ける。
一方、アプリケーションレイヤー等の上位レイヤーが`Renderer Backend`の機能を使用するための窓口はシンプルにしたかったため、Contextは1個にした。
なお、VAO, VBO, Shader, Textureは、同一のグラフィックスAPIを使用することを前提としており、セットで選択する(混在は想定していない)。

この前提を踏まえたStrategyパターンは以下のような構造を取る。

![strategy](./strategy.png)

GLCEにおける各モジュールと、Strategyのオブジェクトの対応は以下のようになっている。概念上のContextは1つである。ただし、公開APIヘッダは見通しを良くするため、機能別に分割している。

| Strategy Object  | GLCE Module                                       | 役割                                                                                                        |
| ---------------- | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Context(*1)      | renderer_backend_context/renderer_backend_context | 上位層に`Renderer Backend`が保有する機能のうち、`Renderer Backend`の初期化、終了のAPI窓口を提供する                  |
|                  | renderer_backend_context/context_shader           | 上位層に`Renderer Backend`が保有する機能のうち、`Shader`関連機能のAPI窓口を提供する                                  |
|                  | renderer_backend_context/context_texture          | 上位層に`Renderer Backend`が保有する機能のうち、`Texture`関連機能のAPI窓口を提供する                                 |
...
| TextureInterface | renderer_backend_interface/interface_texture      | ContextにグラフィックスAPIごとに差し替え可能なTexture関連機能の仮想関数テーブル(Texture機能を抽象化したAPIを保持)を提供する |
...
| TextureConcrete1 | renderer_backend_concretes/gl33/concrete_texture  | InterfaceにOpenGL3.3実装版vtableと、その内部実装を提供する                                                        |
...
| TextureConcrete2 | Not implemented                                   | 対応グラフィックスAPIが増えた際に追加する                                                                          |
| VAOConcrete2     | Not implemented                                   | 対応グラフィックスAPIが増えた際に追加する                                                                          |
| VBOConcrete2     | Not implemented                                   | 対応グラフィックスAPIが増えた際に追加する                                                                          |

*1: Contextは、外部公開API定義用ヘッダを公開APIの見通しを良くするためcontext, context_shader, context_texture, context_vao, context_vboに分割しているが、実装は全てrenderer_backend_context/context.cに記載している。

### Concrete(Shader, Texture, VAO, VBO)の選択(現状)

現状では`Renderer Backend`のRenderer Backend Context初期化時にグラフィックスAPIを指定することで使用するグラフィックスAPIを選択している。

### Concrete(Shader, Texture, VAO, VBO)の選択(将来)
