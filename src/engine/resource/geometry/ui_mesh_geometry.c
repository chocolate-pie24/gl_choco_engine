...
struct ui_mesh_geometry {
    ...
    size_t vertex_count;        /**< ui_mesh_geometryが所有する頂点数(UI矩形領域(三角形2枚)の端点をverticesには格納するので、必ず6の倍数) */
...
resource_result_t ui_mesh_geometry_create_from_vertices(const char* name_, size_t vertex_count_, const ui_vertex_t* vertices_, ui_mesh_geometry_t** geometry_) {
    ...
    // NOTE: vertex_count_が6の倍数かのチェックはui_mesh_geometry_initialize_from_verticesで行う
...
void ui_mesh_geometry_destroy(ui_mesh_geometry_t** geometry_) {
    ...
    } else if(NULL != (*geometry_)->vertices && 0 != ((*geometry_)->vertex_count % 6)) {
        ERROR_MESSAGE("ui_mesh_geometry_destroy(%s) - ui_mesh_geometry internal state is inconsistent: vertex_count is not a multiple of 6. CPU-side vertex array was not freed because allocation size cannot be trusted.", resource_rslt_to_str(RESOURCE_DATA_CORRUPTED));
...
resource_result_t ui_mesh_geometry_initialize_from_vertices(const char* name_, size_t vertex_count_, const ui_vertex_t* vertices_, ui_mesh_geometry_t* geometry_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(0 != vertex_count_, ret, RESOURCE_INVALID_ARGUMENT, resource_rslt_to_str(RESOURCE_INVALID_ARGUMENT), "ui_mesh_geometry_initialize_from_vertices", "vertex_count_")
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(0 == (vertex_count_ % 6), ret, RESOURCE_INVALID_ARGUMENT, resource_rslt_to_str(RESOURCE_INVALID_ARGUMENT), "ui_mesh_geometry_initialize_from_vertices", "vertex_count_")
...
void ui_mesh_geometry_deinitialize(ui_mesh_geometry_t* geometry_) {
    ...
    if(NULL != geometry_->vertices && 0 == geometry_->vertex_count) {
        ...
        ERROR_MESSAGE("ui_mesh_geometry_deinitialize(%s) - ui_mesh_geometry internal state is inconsistent: vertex_count is not a multiple of 6.", resource_rslt_to_str(RESOURCE_DATA_CORRUPTED));
    } else if(NULL != geometry_->vertices) {
        memory_system_free(geometry_->vertices, sizeof(ui_vertex_t) * geometry_->vertex_count, MEMORY_TAG_GEOMETRY);
        geometry_->vertices = NULL;
        geometry_->vertex_count = 0;
...
resource_result_t ui_mesh_geometry_vertices_get(const ui_mesh_geometry_t* geometry_, const ui_vertex_t** out_vertices_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(0 != geometry_->vertex_count, ret, RESOURCE_BAD_OPERATION, resource_rslt_to_str(RESOURCE_BAD_OPERATION), "ui_mesh_geometry_vertices_get", "geometry_->vertex_count")
    IF_ARG_FALSE_GOTO_CLEANUP(0 == (geometry_->vertex_count % 6), ret, RESOURCE_DATA_CORRUPTED, resource_rslt_to_str(RESOURCE_DATA_CORRUPTED), "ui_mesh_geometry_vertices_get", "geometry_->vertex_count")
...
resource_result_t ui_mesh_geometry_vertex_count_get(const ui_mesh_geometry_t* geometry_, size_t* out_vertex_count_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(0 != geometry_->vertex_count, ret, RESOURCE_BAD_OPERATION, resource_rslt_to_str(RESOURCE_BAD_OPERATION), "ui_mesh_geometry_vertex_count_get", "geometry_->vertex_count")
    IF_ARG_FALSE_GOTO_CLEANUP(0 == (geometry_->vertex_count % 6), ret, RESOURCE_DATA_CORRUPTED, resource_rslt_to_str(RESOURCE_DATA_CORRUPTED), "ui_mesh_geometry_vertex_count_get", "geometry_->vertex_count")
