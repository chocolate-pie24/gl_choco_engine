...
// 外部公開APIテスト設定
static test_call_control_t s_test_config_render_mem_allocate;   /**< render_mem_allocate()テスト設定 */
...
#ifdef TEST_BUILD
    s_test_config_render_mem_allocate.call_count++;
    if(s_test_config_render_mem_allocate.fail_on_call != 0) {
        if(s_test_config_render_mem_allocate.call_count == s_test_config_render_mem_allocate.fail_on_call) {
            return (renderer_result_t)s_test_config_render_mem_allocate.forced_result;
        }
    }
#endif
...
#ifdef TEST_BUILD
void NO_COVERAGE test_render_mem_allocate_config_set(const test_call_control_t* config_) {
    if(NULL == config_) {
        assert(false);
        return;
    }
    s_test_config_render_mem_allocate.fail_on_call = config_->fail_on_call;
    s_test_config_render_mem_allocate.forced_result = config_->forced_result;
}

void NO_COVERAGE test_renderer_memory_config_reset(void) {
    test_call_control_reset(&s_test_config_render_mem_allocate);
}
...
static void NO_COVERAGE test_render_mem_allocate(void) {
    {
        // render_mem_allocate() 冒頭で強制的に RENDERER_BAD_OPERATION を返させる
    ...
    {
        // render_mem_allocate() の2回目呼び出しで強制的に RENDERER_LIMIT_EXCEEDED を返させる
        renderer_result_t ret = RENDERER_UNDEFINED_ERROR;
        test_call_control_t config = { 0 };
        void* ptr = NULL;
    ...
    }
    {
        ...
        ret = render_mem_allocate(128U, NULL);
    ...
    }
    {
        ...
        ret = render_mem_allocate(0U, &ptr);
