...
resource_registry_result_t ui_mesh_geometry_registry_draw_range_get(const ui_mesh_geometry_registry_t* registry_, int16_t geometry_id_, size_t* out_vertex_offset_, size_t* out_vertex_count_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_id_is_valid(registry_, geometry_id_), ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "ui_mesh_geometry_registry_draw_range_get", "geometry_id_")

    if(NULL == registry_->geometries[geometry_id_]) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_draw_range_get(%s) - Failed to get ui mesh geometry draw range. reason=not_registered, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }

    ret_resource = ui_mesh_geometry_vertex_count_get(registry_->geometries[geometry_id_], &tmp_count);
    if(RESOURCE_SUCCESS != ret_resource) {
        ret = resource_registry_rslt_convert_resource(ret_resource);
        ERROR_MESSAGE("ui_mesh_geometry_registry_draw_range_get(%s) - Failed to get ui mesh geometry draw range. reason=vertex_count_get_failed, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }
