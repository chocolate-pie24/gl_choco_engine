 ...
 * @brief 点群を描画するための、point_mesh_geometry幾何情報のCPUリソースとGPUリソースをIDを用いて管理するシステムの実装
 *
 * @version 0.1
 * @date 2026-06-20
 *
...
#include "engine/systems/renderer/resource_registries/point_mesh_geometry_registry.h"
...
/**
 * @brief point_mesh_geometry_t管理システム構造体
 *
 * @note GLCEはcolorは幾何情報ではないのでgeometryでは扱わない(material側で管理する)
 *
 */
struct point_mesh_geometry_registry {
    ...
    // CPU resources
    point_mesh_geometry_t** geometries; /**< geometry CPUリソース構造体へのポインタ配列(リソース所有権はregistry) */
...
static bool geometry_registry_internal_state_check(const point_mesh_geometry_registry_t* registry_);
static bool point_mesh_geometry_find(const char* name_, const point_mesh_geometry_registry_t* registry_, size_t* out_index_);
...
resource_registry_result_t point_mesh_geometry_registry_initialize(size_t max_geometry_count_, linear_alloc_t* allocator_, point_mesh_geometry_registry_t** out_registry_) {
    ...
    if((SIZE_MAX / max_geometry_count_) < sizeof(size_t)) {
        ret = RESOURCE_REGISTRY_OVERFLOW;
        ERROR_MESSAGE("point_mesh_geometry_registry_initialize(%s) - Allocation size overflow while calculating vertex offset array size. target=vertex_offsets, elem_type=size_t, elem_count=%zu, elem_size=%zu, size_max=%zu", resource_registry_rslt_to_str(ret), max_geometry_count_, sizeof(size_t), SIZE_MAX);
        goto cleanup;
    }
    ...
    if(LINEAR_ALLOC_SUCCESS != ret_linear_alloc) {
        ret = resource_registry_rslt_convert_linear_alloc(ret_linear_alloc);
        ERROR_MESSAGE("point_mesh_geometry_registry_initialize(%s) - Failed to allocate vertex offset array. target=vertex_offsets, elem_type=size_t, elem_count=%zu, elem_size=%zu, bytes=%zu, align=%zu", resource_registry_rslt_to_str(ret), max_geometry_count_, sizeof(size_t), sizeof(size_t) * max_geometry_count_, alignof(size_t));
        goto cleanup;
    }
...
resource_registry_result_t point_mesh_geometry_registry_geometry_id_get(const char* name_, const point_mesh_geometry_registry_t* registry_, int16_t* out_geometry_id_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_registry_internal_state_check(registry_), ret, RESOURCE_REGISTRY_DATA_CORRUPTED, resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), "point_mesh_geometry_registry_geometry_id_get", "registry_")
...
resource_registry_result_t point_mesh_geometry_registry_draw_range_get(int16_t geometry_id_, const point_mesh_geometry_registry_t* registry_, size_t* out_vertex_offset_, size_t* out_vertex_count_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(registry_, ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "point_mesh_geometry_registry_draw_range_get", "registry_")
    IF_ARG_NULL_GOTO_CLEANUP(out_vertex_offset_, ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "point_mesh_geometry_registry_draw_range_get", "out_vertex_offset_")
    IF_ARG_NULL_GOTO_CLEANUP(out_vertex_count_, ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "point_mesh_geometry_registry_draw_range_get", "out_vertex_count_")
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_registry_internal_state_check(registry_), ret, RESOURCE_REGISTRY_DATA_CORRUPTED, resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), "point_mesh_geometry_registry_draw_range_get", "registry_")
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_id_valid_check(geometry_id_, registry_), ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "point_mesh_geometry_registry_draw_range_get", "geometry_id_")

    if(NULL == registry_->geometries[geometry_id_]) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("point_mesh_geometry_registry_draw_range_get(%s) - Failed to get point mesh geometry draw range. reason=not_registered, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }

    ret_resource = point_mesh_geometry_vertex_count_get(registry_->geometries[geometry_id_], &tmp_count);
    if(RESOURCE_SUCCESS != ret_resource) {
        ret = resource_registry_rslt_convert_resource(ret_resource);
        ERROR_MESSAGE("point_mesh_geometry_registry_draw_range_get(%s) - Failed to get point mesh geometry draw range. reason=vertex_count_get_failed, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }
...
resource_registry_result_t point_mesh_geometry_registry_geometry_register(const point_mesh_geometry_t* geometry_, size_t vertex_offset_, point_mesh_geometry_registry_t* registry_, int16_t* out_geometry_id_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_registry_internal_state_check(registry_), ret, RESOURCE_REGISTRY_DATA_CORRUPTED, resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), "point_mesh_geometry_registry_geometry_register", "registry_")
    ...
    for(size_t i = 0; i != registry_->max_geometry_count; ++i) {
        if(NULL == registry_->geometries[i]) {
            ret_resource = point_mesh_geometry_clone(geometry_, &new_point_mesh_geometry);
            if(RESOURCE_SUCCESS != ret_resource) {
                ret = resource_registry_rslt_convert_resource(ret_resource);
    ...
    registry_->geometries[free_slot] = new_point_mesh_geometry;
...
resource_registry_result_t point_mesh_geometry_registry_geometry_unregister(int16_t geometry_id_, point_mesh_geometry_registry_t* registry_) {
    ...
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_registry_internal_state_check(registry_), ret, RESOURCE_REGISTRY_DATA_CORRUPTED, resource_registry_rslt_to_str(RESOURCE_REGISTRY_DATA_CORRUPTED), "point_mesh_geometry_registry_geometry_unregister", "registry_")
    IF_ARG_FALSE_GOTO_CLEANUP(geometry_id_valid_check(geometry_id_, registry_), ret, RESOURCE_REGISTRY_INVALID_ARGUMENT, resource_registry_rslt_to_str(RESOURCE_REGISTRY_INVALID_ARGUMENT), "point_mesh_geometry_registry_geometry_unregister", "geometry_id_")
