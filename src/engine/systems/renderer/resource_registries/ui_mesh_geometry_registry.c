...
#include "engine/systems/renderer/resource_registries/ui_mesh_geometry_registry.h"
...
resource_registry_result_t ui_mesh_geometry_registry_geometry_id_get(const char* name_, const ui_mesh_geometry_registry_t* registry_, int16_t* out_geometry_id_) {
    ...
    if(!ui_mesh_geometry_find(name_, registry_, &tmp_id)) {
        ...
        ERROR_MESSAGE("ui_mesh_geometry_registry_geometry_id_get(%s) - Failed to get ui mesh geometry id. reason=not_registered, query_name='%s'", resource_registry_rslt_to_str(ret), name_);
        goto cleanup;
    }
...
resource_registry_result_t ui_mesh_geometry_registry_geometry_register(const ui_mesh_geometry_t* geometry_, size_t vertex_offset_, ui_mesh_geometry_registry_t* registry_, int16_t* out_geometry_id_) {
    ...
    if(NULL == name) {
        ...
        ERROR_MESSAGE("ui_mesh_geometry_registry_geometry_register(%s) - Failed to register ui mesh geometry. reason=name_get_failed", resource_registry_rslt_to_str(ret));
        goto cleanup;
    }
    if(ui_mesh_geometry_find(name, registry_, &rubbish)) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_geometry_register(%s) - Failed to register ui mesh geometry. reason=already_registered, geometry_name='%s'", resource_registry_rslt_to_str(ret), name);
        goto cleanup;
    }
    ...
    if(!found_free_slot) {
        ...
        ERROR_MESSAGE("ui_mesh_geometry_registry_geometry_register(%s) - Failed to register ui mesh geometry. reason=registry_full, geometry_name='%s', max_geometry_count=%zu", resource_registry_rslt_to_str(ret), name, registry_->max_geometry_count);
        goto cleanup;
    }
...
resource_registry_result_t ui_mesh_geometry_registry_geometry_unregister(int16_t geometry_id_, ui_mesh_geometry_registry_t* registry_) {
    ...
    if(NULL == registry_->geometries[geometry_id_]) {
        ret = RESOURCE_REGISTRY_BAD_OPERATION;
        ERROR_MESSAGE("ui_mesh_geometry_registry_geometry_unregister(%s) - Failed to unregister ui mesh geometry. reason=not_registered, geometry_id=%d", resource_registry_rslt_to_str(ret), geometry_id_);
        goto cleanup;
    }
