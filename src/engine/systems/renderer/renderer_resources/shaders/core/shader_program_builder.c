...
renderer_result_t shader_program_builder_create_from_files(renderer_backend_context_t* backend_context_, const char* file_path_, const char* name_, renderer_backend_shader_t** out_shader_) {
...
static renderer_result_t shader_source_load(const char* file_path_, const char* name_, const char* extension_, choco_string_t** out_shader_source_) {
    ...
    if(CHOCO_STRING_SUCCESS != ret_string) {
        ...
        ERROR_MESSAGE("shader_source_load(%s) - Failed to create string for shader_source.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(FS_UTILS_SUCCESS != ret_fs_utils) {
        ...
        ERROR_MESSAGE("shader_source_load(%s) - Failed to create fs_utils for shader_source.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(FS_UTILS_SUCCESS != ret_fs_utils) {
        ret = renderer_rslt_convert_fs_utils(ret_fs_utils);
        ERROR_MESSAGE("shader_source_load(%s) - Failed to read shader source.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
