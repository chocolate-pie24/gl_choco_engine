...
renderer_result_t lit_mesh_shader_program_initialize(renderer_backend_context_t* backend_context_, lit_mesh_shader_t* lit_mesh_shader_, const char* file_path_, const char* name_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(backend_context_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_program_initialize", "backend_context_")
    IF_ARG_NULL_GOTO_CLEANUP(lit_mesh_shader_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_program_initialize", "lit_mesh_shader_")
    IF_ARG_NULL_GOTO_CLEANUP(file_path_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_program_initialize", "file_path_")
    IF_ARG_NULL_GOTO_CLEANUP(name_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_program_initialize", "name_")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(lit_mesh_shader_->shader, ret, RENDERER_BAD_OPERATION, renderer_rslt_to_str(RENDERER_BAD_OPERATION), "lit_mesh_shader_program_initialize", "lit_mesh_shader_->shader")

    // シェーダープログラムビルド
    ret = shader_program_builder_create_from_files(backend_context_, file_path_, name_, &tmp_shader);
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_program_initialize(%s) - Failed to build shader program.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_program_initialize(%s) - Failed to get model matrix location.", renderer_rslt_to_str(ret));
        goto cleanup;
    }

    ret = renderer_backend_shader_uniform_location_get(backend_context_, tmp_shader, "g_view_matrix", &tmp_view_matrix_location);
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_program_initialize(%s) - Failed to get view matrix location.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    ...
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_program_initialize(%s) - Failed to get projection matrix location.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
...
renderer_result_t lit_mesh_shader_vbo_initialize(renderer_backend_context_t* backend_context_, lit_mesh_shader_t* lit_mesh_shader_, const vbo_manager_config_t* vbo_config_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(backend_context_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_initialize", "backend_context_")
    IF_ARG_NULL_GOTO_CLEANUP(lit_mesh_shader_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_initialize", "lit_mesh_shader_")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(lit_mesh_shader_->vao, ret, RENDERER_BAD_OPERATION, renderer_rslt_to_str(RENDERER_BAD_OPERATION), "lit_mesh_shader_vbo_initialize", "lit_mesh_shader_->vao")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(lit_mesh_shader_->vbo_manager, ret, RENDERER_BAD_OPERATION, renderer_rslt_to_str(RENDERER_BAD_OPERATION), "lit_mesh_shader_vbo_initialize", "lit_mesh_shader_->vbo_manager")
    IF_ARG_NULL_GOTO_CLEANUP(vbo_config_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_initialize", "vbo_config_")
    IF_ARG_FALSE_GOTO_CLEANUP(vbo_config_is_valid(vbo_config_), ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_initialize", "vbo_config_")
    ...
    if(BUFFER_MANAGER_SUCCESS != ret_buff_mgr) {
        ...
        ERROR_MESSAGE("lit_mesh_shader_vbo_initialize(%s) - buffer manager create failed.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
...
renderer_result_t lit_mesh_shader_vao_initialize(renderer_backend_context_t* backend_context_, lit_mesh_shader_t* lit_mesh_shader_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(backend_context_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vao_initialize", "backend_context_")
    IF_ARG_NULL_GOTO_CLEANUP(lit_mesh_shader_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vao_initialize", "lit_mesh_shader_")
    IF_ARG_NOT_NULL_GOTO_CLEANUP(lit_mesh_shader_->vao, ret, RENDERER_BAD_OPERATION, renderer_rslt_to_str(RENDERER_BAD_OPERATION), "lit_mesh_shader_vao_initialize", "lit_mesh_shader_->vao")
    IF_ARG_NULL_GOTO_CLEANUP(lit_mesh_shader_->vbo_manager, ret, RENDERER_BAD_OPERATION, renderer_rslt_to_str(RENDERER_BAD_OPERATION), "lit_mesh_shader_vao_initialize", "lit_mesh_shader_->vbo_manager")

    ret = renderer_backend_vertex_array_create(backend_context_, &lit_mesh_shader_->vao);
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_vao_initialize(%s) - Failed to create lit mesh vao.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    vao_created = true;

    ret = renderer_backend_vertex_array_bind(backend_context_, lit_mesh_shader_->vao);
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_vao_initialize(%s) - Failed to bind vertex array.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    vao_bound = true;
    ...
    if(BUFFER_MANAGER_SUCCESS != ret_buff_mgr) {
        ...
        ERROR_MESSAGE("lit_mesh_shader_vao_initialize(%s) - Failed to bind vertex buffer.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    vbo_bound = true;
    ...
    if(BUFFER_MANAGER_SUCCESS != ret_buff_mgr) {
        // TODO: buffer_manager仕様確定後、実行結果コード変換を適切にする
        ret = RENDERER_RUNTIME_ERROR;
        ERROR_MESSAGE("lit_mesh_shader_vao_initialize(%s) - Failed to unbind vertex buffer.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    vbo_bound = false;
    ...
    if(RENDERER_SUCCESS != ret) {
        ERROR_MESSAGE("lit_mesh_shader_vao_initialize(%s) - Failed to unbind vertex array.", renderer_rslt_to_str(ret));
        goto cleanup;
    }
    vao_bound = false;
...
renderer_result_t lit_mesh_shader_vbo_write(const renderer_backend_context_t* backend_context_, lit_mesh_shader_t* lit_mesh_shader_, size_t size_, const point_normal_vertex_t* write_data_, vertex_buffer_range_t* out_buffer_range_) {
    renderer_result_t ret = RENDERER_INVALID_ARGUMENT;
    ...
    IF_ARG_NULL_GOTO_CLEANUP(backend_context_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_write", "backend_context_")
    IF_ARG_FALSE_GOTO_CLEANUP(lit_mesh_shader_is_initialized(lit_mesh_shader_), ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_write", "lit_mesh_shader_")
    IF_ARG_NULL_GOTO_CLEANUP(write_data_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_write", "write_data_")
    IF_ARG_FALSE_GOTO_CLEANUP(0 != size_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_write", "size_")
    IF_ARG_FALSE_GOTO_CLEANUP(0 == (size_ % (sizeof(point_normal_vertex_t) * 3)), ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_write", "size_")
    IF_ARG_NULL_GOTO_CLEANUP(out_buffer_range_, ret, RENDERER_INVALID_ARGUMENT, renderer_rslt_to_str(RENDERER_INVALID_ARGUMENT), "lit_mesh_shader_vbo_write", "out_buffer_range_")

    ret_buff_mgr = vbo_manager_write(lit_mesh_shader_->vbo_manager, backend_context_, size_, (const void*)write_data_, &tmp_alloc_handle);
    if(BUFFER_MANAGER_SUCCESS != ret_buff_mgr) {
        ret = RENDERER_RUNTIME_ERROR;   // TODO: buffer_managerの仕様が安定したら適切な実行結果コードに変換する
        ERROR_MESSAGE("lit_mesh_shader_vbo_write(%s) - vbo write failed.", renderer_rslt_to_str(ret));
        goto cleanup;
    }

    out_buffer_range_->allocation_size = tmp_alloc_handle.allocated_size;
    out_buffer_range_->draw_range.first_vertex_count = tmp_alloc_handle.byte_offset / sizeof(point_normal_vertex_t);
    out_buffer_range_->draw_range.vertex_count = size_ / sizeof(point_normal_vertex_t);
...
renderer_result_t lit_mesh_shader_vbo_free(lit_mesh_shader_t* lit_mesh_shader_, const vertex_buffer_range_t* buffer_range_) {
    ...
    alloc_info.byte_offset = buffer_range_->draw_range.first_vertex_count * sizeof(point_normal_vertex_t);
...
renderer_result_t lit_mesh_shader_vertex_array_bind(const renderer_backend_context_t* backend_context_, const lit_mesh_shader_t* lit_mesh_shader_) {
    ...
    IF_ARG_NULL_GOTO_CLEANUP(lit_mesh_shader_->vao, ret, RENDERER_BAD_OPERATION, renderer_rslt_to_str(RENDERER_BAD_OPERATION), "lit_mesh_shader_vertex_array_bind", "vao")
