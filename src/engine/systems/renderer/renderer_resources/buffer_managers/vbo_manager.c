...
buffer_manager_result_t vbo_manager_create(renderer_backend_context_t* backend_context_, const vbo_manager_config_t* config_, vbo_manager_t** out_vbo_manager_) {
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ...
        ERROR_MESSAGE("vbo_manager_create(%s) - Failed to create VBO Manager. reason=renderer_backend_vertex_buffer_create_failed, renderer_result=%d", buffer_manager_rslt_to_str(ret), (int)ret_renderer);
        goto cleanup;
    }
    vbo_created = true;
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_create(%s) - Failed to create VBO Manager. reason=renderer_backend_vertex_buffer_bind_failed, renderer_result=%d", buffer_manager_rslt_to_str(ret), (int)ret_renderer);
        goto cleanup;
    }
    vbo_bound = true;
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_create(%s) - Failed to create VBO Manager. reason=renderer_backend_vertex_buffer_vertex_load_failed, vbo_size=%zu, buffer_usage=%d, renderer_result=%d", buffer_manager_rslt_to_str(ret), config_->vbo_size, (int)config_->buffer_usage, (int)ret_renderer);
        goto cleanup;
    }
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_create(%s) - Failed to create VBO Manager. reason=renderer_backend_vertex_buffer_unbind_failed, renderer_result=%d", buffer_manager_rslt_to_str(ret), (int)ret_renderer);
        goto cleanup;
    }
    vbo_bound = false;
    ...
    if(BUFFER_MANAGER_SUCCESS != ret) {
        if(vbo_created) {
            if(vbo_bound) {
                ret_renderer = renderer_backend_vertex_buffer_unbind(backend_context_);
                if(RENDERER_SUCCESS != ret_renderer) {
                    ERROR_MESSAGE("vbo_manager_create(%s) - Failed to rollback VBO Manager creation. reason=renderer_backend_vertex_buffer_unbind_failed, original_result=%s, renderer_result=%d", buffer_manager_rslt_to_str(BUFFER_MANAGER_DATA_CORRUPTED), buffer_manager_rslt_to_str(ret), (int)ret_renderer);
...
buffer_manager_result_t vbo_manager_write(vbo_manager_t* vbo_manager_, const renderer_backend_context_t* backend_context_, size_t size_, const void* write_data_, vertex_allocation_t* out_allocation_handle_) {
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_write(%s) - Failed to write vertex data. reason=renderer_backend_vertex_buffer_bind_failed, allocation_offset=%zu, allocation_size=%zu, renderer_result=%d", buffer_manager_rslt_to_str(ret), tmp_allocation.offset, tmp_allocation.allocated_size, (int)ret_renderer);
        goto cleanup;
    }
    vbo_bound = true;
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_write(%s) - Failed to write vertex data. reason=renderer_backend_vertex_buffer_vertex_subload_failed, write_offset=%zu, write_size=%zu, renderer_result=%d", buffer_manager_rslt_to_str(ret), tmp_allocation.offset, size_, (int)ret_renderer);
        goto cleanup;
    }
    load_success = true;
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_write(%s) - Failed to write vertex data. reason=renderer_backend_vertex_buffer_unbind_failed, allocation_offset=%zu, allocation_size=%zu, renderer_result=%d", buffer_manager_rslt_to_str(ret), tmp_allocation.offset, tmp_allocation.allocated_size, (int)ret_renderer);
        goto cleanup;
    }
    vbo_bound = false;
...
cleanup:
    if(allocate_success) {
        ...
        if(vbo_bound) {
            ...
            ret_renderer = renderer_backend_vertex_buffer_unbind(backend_context_);
            if(RENDERER_SUCCESS != ret_renderer) {
                ERROR_MESSAGE("vbo_manager_write(%s) - Failed to rollback vertex data write. reason=renderer_backend_vertex_buffer_unbind_failed, previous_result=%s, allocation_offset=%zu, allocation_size=%zu, renderer_result=%d", buffer_manager_rslt_to_str(BUFFER_MANAGER_DATA_CORRUPTED), buffer_manager_rslt_to_str(ret), tmp_allocation.offset, tmp_allocation.allocated_size, (int)ret_renderer);
                ret = BUFFER_MANAGER_DATA_CORRUPTED;
            }
        }
    }
...
buffer_manager_result_t vbo_manager_bind(vbo_manager_t* vbo_manager_, const renderer_backend_context_t* backend_context_) {
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_bind(%s) - Failed to bind vertex buffer. reason=renderer_backend_vertex_buffer_bind_failed, renderer_result=%d", buffer_manager_rslt_to_str(ret), (int)ret_renderer);
        goto cleanup;
    }
...
buffer_manager_result_t vbo_manager_unbind(const renderer_backend_context_t* backend_context_) {
    ...
    if(RENDERER_SUCCESS != ret_renderer) {
        ret = buffer_manager_rslt_convert_renderer(ret_renderer);
        ERROR_MESSAGE("vbo_manager_unbind(%s) - Failed to unbind vertex buffer. reason=renderer_backend_vertex_buffer_unbind_failed, renderer_result=%d", buffer_manager_rslt_to_str(ret), (int)ret_renderer);
        goto cleanup;
    }
