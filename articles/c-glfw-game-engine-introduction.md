...
Book2 step2_6「このステップでやること」に記載していた、ウィンドウ生成処理を行う関数名の誤記修正(application_run -> application_create)

### 2025-11-22

Book1 step1_0 対応プラットフォームについて「Windowsは未定」->「Windowsもサポート」に修正

### 2025-11-22

GL Choco Engineの立ち位置を本記事に追加し、それに伴い全体を整理

### 2025-11-22

Book3(2d-rendering-step3)リリース

### 2026-06-16

開発を進めているGL Choco Engineですが、関数名・引数・データ構造などの公開インターフェイスに変更が続いており、すでに記事の内容と最新の実装が一致しない状態が頻発しています。
仕様が変更するたびに過去の内容を全て修正したり、修正点を全て記載するのは現実的ではないため、本シリーズの更新を一時的に停止します。

今後は、エンジン全体の骨格と主要なAPI仕様がある程度安定したタイミングで、各レイヤーごとに内容を整理し直して更新する予定です。なお、現時点の記事は「執筆時点での設計・実装メモ」としてお読みください。
...
- [Book4: シンプルな三角形の描画とレンダラーバックエンドの構築](https://zenn.dev/chocolate_pie24/books/2d_rendering_step4)
- [Book5: MVP行列の導入と、カメラシステムの構築](https://zenn.dev/chocolate_pie24/books/2d_rendering_step5)
